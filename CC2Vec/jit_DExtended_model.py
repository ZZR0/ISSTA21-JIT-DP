...
class DeepJITExtended(nn.Module):
    def __init__(self, args):
        ...
        if self.args.code:
            fcin += len(Ks) * Co
        self.fc1 = nn.Linear(fcin, args.hidden_units)  # hidden units
    ...
    def forward(self, ftr, msg, code):
        ...
        if self.args.ftr:
            x_commit = ftr
        if self.args.msg:
            x_commit = torch.cat((x_commit, x_msg), 1) if x_commit != None else x_msg
        if self.args.code:
            x_commit = torch.cat((x_commit, x_code), 1) if x_commit != None else x_code
